...
from model.scheam.commit_scheam import Comment_scheam
from model.models.model import Comment
...
class Comment_router(APIRouter):
    ...
    async def comment_list(self, type_: int, auth_name: str = None, start_time: str = None, end_time: str = None, sort_by: str = 'create_time',sort_order: str = 'asc',current: int = 1,page_size: int = 20,db: Session = Depends(get_db),):
        # 搜索文章评论
        search_dict = {
            'curd': False,  # 搜索的字段
        ...
        return await BaseCurd(db, Comment).query_(search_dict)

    async def comment_add(self, item: Comment_scheam, db: Session = Depends(get_db)):
        # 添加评论
        return BaseCurd(db, Comment).create_({'curd': item.dict(), 'is_commit': True})

    async def comment_change(self, id: int, item: Comment_scheam, db: Session = Depends(get_db)):
        # 修改评论
        return BaseCurd(db, Comment).update_({'query': {'id': id}, 'curd': item.dict(), 'is_commit': True})

    async def comment_remove(self, id: int, db: Session = Depends(get_db)):
        # 删除评论
        return BaseCurd(db, Comment).update_({'query': {'id': id}, 'curd': {'status':0}, 'is_commit': True})
